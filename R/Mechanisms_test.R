...
test_Gaussian <- function(){
  ...
  graphics::hist(data,freq=FALSE,main="Simple example (aDP)");
  x = seq(tv-5*th.s, tv+5*th.s,.1);
  graphics::lines(x,stats::dnorm(x,m=tv,sd=th.s));
  print("Verify line matches histogram...")
  Sys.sleep(2)

  print("         Simple example (analytic):");
  tv = 0;
  eps = .5;
  delta = .01;
  sens = 1;
  ...
  data = numeric(n);
  for (i in 1:n){
    data[i] = GaussianMechanism(tv,eps,delta,sens,alloc.proportions=ap,
                                analytic=analytic);
  }
  graphics::hist(data,freq=FALSE,main="Simple example (analytic)");
  x = seq(tv-5*th.s, tv+5*th.s,.1);
  graphics::lines(x,stats::dnorm(x,m=tv,sd=th.s));
  print("Verify line matches histogram...")
  Sys.sleep(2)
