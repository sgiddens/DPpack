...
#
# error <- sum(abs(lrdp$predict(X,add.bias=TRUE)-y))/length(y)
...
#   #ggtitle('Data to be classified') +
#   theme_bw(base_size = 12) + xlim(-.8,.8) + ylim(-.7,.7)
#   #theme(legend.position=c(0.85, 0.87)) #+ geom_path(data=cir, aes(x,y))
#
# set.seed(round(runif(1,0,100)))
#
# # Hard-margin
...
# D <- 20
#
# # KernelSupportVectorMachineDP$undebug("preprocess_data")
# # ksvmdp <- KernelSupportVectorMachineDP$new('l2', eps, lambda, D)
# ksvmdp <- SupportVectorMachineDP$new('l2', eps, lambda)
#
...
#   coord_fixed(ratio = 1) +
#   theme_bw(base_size = 12) + xlim(-.8,.8) + ylim(-.7,.7)
# #+geom_line(data=my.data,aes(x=my.x,y=my.y))+
# # geom_line(data=my.data2,aes(x=my.x2,y=my.y2)) +
# # geom_point(data=data.frame(t(colMeans(data[,1:2]))), aes(x=x1,y=x2))
#
# # Standard SVM
...
# # predict the label
# # Z <- logisticPred(probZ)
# Z <- sign(predict(stdSVM, newdata=grid,decision.values=TRUE)-.5)
# gridPred = cbind(grid, Z)
# colnames(gridPred)[3] <- 'label'
# gridPred <- data.frame(gridPred)
#
...
#
# #######################################################
# ### TESTING KERNEL SVM AND COMPARING TO REGULAR SVM ### (Nonlinear)
# #######################################################
# # Build dataset
...
#
# # Standard SVM
# stdSVM <- svm(as.matrix(X),y,scale=FALSE,kernel="radial",gamma=gamma,cost=lambda,
#               probability=TRUE)
#
# # generate a grid for decision boundary, this is the test set
# grid <- expand.grid(seq(-1, 1, length.out = 100), seq(-1, 1, length.out = 100))
...
# # predict the label
# # Z <- logisticPred(probZ)
# Z <- sign(predict(stdSVM, newdata=grid)-.5)
# gridPred = cbind(grid, Z)
# colnames(gridPred)[3] <- 'label'
# gridPred <- data.frame(gridPred)
#
...
#
# # model <- lrdp2
# # model$fit(X, y, upper.bounds,lower.bounds)
